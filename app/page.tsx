...
import { ArrowRight, Sparkles, Target, Briefcase, Award, Rocket, PlayCircle, Star, Users, MessageSquareQuote, CheckCircle2, ChevronRight, ChevronDown } from 'lucide-react';
import { COURSES, TESTIMONIALS, CATEGORIES, formatRupiah } from '@/lib/dummyData';

const FAQ_DATA = [
  {
    id: 1,
    question: "Apakah pemula tanpa background IT atau desain bisa ikut belajar?",
    answer: "Tentu saja! Materi kami disusun khusus dari level fundamental. Kamu akan dibimbing step-by-step dari nol untuk menguasai skill digital modern, baik itu Web Development, UI/UX Design, maupun Digital Marketing, tanpa perlu pengalaman sebelumnya."
  },
  {
    id: 2,
    question: "Apakah sertifikatnya bisa digunakan untuk melamar kerja?",
    answer: "Sangat bisa. Sertifikat Inovasi Digital Academy mencantumkan skill spesifik dan portofolio real-project yang kamu selesaikan. Nilai praktis inilah yang sangat dicari dan disukai oleh rekruter di berbagai startup dan perusahaan digital."
  },
  {
    id: 3,
    question: "Bagaimana jika saya mengalami kesulitan saat mempraktikkan materi?",
    answer: "Kamu tidak akan belajar sendirian. Kami menyediakan forum diskusi komunitas yang aktif dan sesi mentoring eksklusif (khusus paket Pro & Premium). Mentor expert kami siap membantu menjawab pertanyaan dan me-review hasil kerjamu."
  },
  {
    id: 4,
    question: "Berapa lama batas waktu akses materi yang sudah dibeli?",
    answer: "Untuk kelas gratis (Basic), materi dapat diakses selamanya seumur hidup. Sedangkan untuk materi level Pro dan Premium, batas akses akan mengikuti durasi paket berlangganan yang kamu pilih (Bulanan atau Tahunan)."
  }
];
...
export default function HomePage() {
  ...
  // Tambahan useRef untuk mengakali pembulatan desimal browser
  const exactScrollPos = useRef<number>(0); 
  ...
  // Perbanyak duplikasi agar loopnya benar-benar terasa tidak putus
  const marqueeCategories = [...CATEGORIES, ...CATEGORIES, ...CATEGORIES, ...CATEGORIES, ...CATEGORIES, ...CATEGORIES, ...CATEGORIES, ...CATEGORIES];

  const filteredCourses = COURSES.filter(course => {
    return selectedCategory === "Semua Kelas" || course.category === selectedCategory;
  });
  
  const displayedCourses = filteredCourses.slice(0, 3);
  const hasMoreCourses = filteredCourses.length > 3;

  // LOGIKA AUTO SCROLL YANG DIPERBAIKI
  useEffect(() => {
    ...
    const playScroll = () => {
      if (container && !isDragging && !isHovered) {
        // Kita tambah nilai ke ref eksternal agar desimal (0.3) tidak hilang
        exactScrollPos.current += 0.3; 
        container.scrollLeft = exactScrollPos.current;

        // Reset logika untuk infinite scroll
        if (container.scrollLeft >= (container.scrollWidth / 2)) {
          container.scrollLeft = 0;
          exactScrollPos.current = 0;
        }
      } else if (container) {
        // Sinkronisasi posisi saat user lagi nge-drag agar tidak lompat
        exactScrollPos.current = container.scrollLeft;
      }
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    exactScrollPos.current = scrollRef.current.scrollLeft; // Sync
  };
    ...
    <div className="animate-in fade-in duration-500 overflow-hidden">
      ...
      <section className="relative pt-24 pb-20 md:pt-36 md:pb-32 px-4 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-8 leading-[1.15]">
            Bangun Karir Digital <br className="hidden md:block"/>
      ...
      <section className="py-24 px-4 max-w-7xl mx-auto border-t border-slate-100 dark:border-slate-800/50">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white mb-4">Kenapa Belajar di Inovasi Digital Academy?</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="group bg-white dark:bg-slate-800/50 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700 hover:border-amber-300 dark:hover:border-amber-500/50 shadow-sm hover:shadow-xl hover:shadow-amber-500/5 transition-all duration-300">
            ...
            <p className="text-slate-500 dark:text-slate-400 leading-relaxed">Kerjakan real-world project di setiap akhir materi untuk memperkuat resume dan portfolio karya kamu.</p>
          </div>
      ...
      <section className="bg-slate-50/50 dark:bg-slate-900/20 py-24 border-y border-slate-200 dark:border-slate-800/50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
            ...
            <Link href="/learning-path" className="px-6 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 text-slate-900 dark:text-white font-medium rounded-xl transition-all shadow-sm flex items-center shrink-0">
          ...
          <div className="mb-12 w-full">
            ...
            >
              <div 
                ...
                className={`flex gap-3 overflow-x-auto hide-scrollbar select-none py-2 ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
              ...
              >
                {marqueeCategories.map((category, index) => (
                  <button
                    ...
                    onClick={() => {
                      if (!isDragging) {
                        setSelectedCategory(selectedCategory === category ? "Semua Kelas" : category);
                      }
                    }}
                    className={`flex-shrink-0 whitespace-nowrap px-6 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 ${
          ...
          {displayedCourses.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-slate-500 dark:text-slate-400">Belum ada kelas yang tersedia untuk kategori ini.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {displayedCourses.map((course, index) => (
                  <Link href={`/course/${course.id}`} key={course.id} className="group bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-1.5 transition-all duration-300 flex flex-col h-full animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${index * 50}ms` }}>
              ...
              {/* Tombol Lihat Selengkapnya (Muncul kalau lebih dari 3 kelas) */}
              {hasMoreCourses && (
                <div className="mt-12 flex justify-center animate-in fade-in slide-in-from-bottom-4">
                  <Link href={`/learning-path?category=${encodeURIComponent(selectedCategory)}`} className="group flex items-center justify-center px-8 py-4 bg-white dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 font-bold rounded-2xl transition-all shadow-sm">
                    Lihat Selengkapnya <ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
                  </Link>
                </div>
              )}
            </>
      ...
      <section className="py-24 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-2xl mx-auto text-lg">Ribuan orang telah membuktikan bahwa Inovasi Digital Academy adalah jalan pintas terbaik menuju karir impian mereka.</p>
        </div>
      ...
      {/* 6. FAQ Section (MODERN ACCORDION STYLE) */}
      <section className="py-24 px-4 border-t border-slate-200 dark:border-slate-800">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-lg">Masih ragu? Berikut jawaban untuk pertanyaan umum dari calon pendaftar kami.</p>
          </div>
              ...
              <div 
                ...
                className={`border rounded-2xl overflow-hidden transition-colors duration-300 ${
                  openFaq === faq.id 
                    ...
                    : 'bg-slate-50/50 dark:bg-slate-800/30 border-slate-200 dark:border-slate-700/50 hover:border-slate-300 dark:hover:border-slate-600'
                }`}
              >
                <button 
                ...
                >
                  <h4 className={`font-bold text-lg pr-4 transition-colors ${
                    openFaq === faq.id ? 'text-blue-600 dark:text-blue-400' : 'text-slate-900 dark:text-white'
                  }`}>
                    {faq.question}
                  </h4>
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-transform duration-300 ${
                    openFaq === faq.id ? 'bg-blue-100 dark:bg-blue-900/50 rotate-180' : 'bg-slate-200 dark:bg-slate-700 rotate-0'
                  }`}>
                    <ChevronDown className={`w-5 h-5 ${openFaq === faq.id ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400'}`} />
                  </div>
                </button>
                ...
                >
                  <div className="overflow-hidden">
                    <p className="px-6 pb-6 md:px-8 md:pb-8 text-slate-600 dark:text-slate-400 leading-relaxed pt-2">
                      {faq.answer}
                    </p>
                  </div>
                </div>
